...
OUT_DIR = "export_wayback"
IMAGES_CSV = "export_wayback/extracted_images.csv"
...
def mark_url_as_processed(filename, url, line_num, status='done', error_msg=None):
    """Označí URL v souboru jako zpracovanou nebo s chybou."""
    try:
        with open(filename, 'r', encoding='utf-8') as f:
            lines = f.readlines()
        
        # najdi správný řádek a označ ho
        if line_num < len(lines):
            original_line = lines[line_num].strip()
            if status == 'done':
                lines[line_num] = f"# DONE: {url}\n"
            elif status == 'error':
                lines[line_num] = f"# ERROR: {url} ({error_msg})\n"
        ...
        print(f"[!] Chyba při označování URL: {e}")
...
def init_progress_files():
    ...
    for filename in [f"{OUT_DIR}/discovered_urls.txt", f"{OUT_DIR}/processed_articles.txt"]:
        if os.path.exists(filename):
            os.remove(filename)
    
    # vytvoř hlavičky
    with open(f"{OUT_DIR}/discovered_urls.txt", 'w', encoding='utf-8') as f:
        f.write(f"# Průběžně objevované URL - spuštěno {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    
    with open(f"{OUT_DIR}/processed_articles.txt", 'w', encoding='utf-8') as f:
        f.write(f"# Zpracované články - spuštěno {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")

def log_processed_article(title, url, filename=None):
    """Zaznamená zpracovaný článek."""
    if filename is None:
        filename = f"{OUT_DIR}/processed_articles.txt"
    with open(filename, 'a', encoding='utf-8') as f:
        f.write(f"{datetime.now().strftime('%H:%M:%S')} - {title} - {url}\n")

def load_urls_from_file(filename="kestazeni.txt"):
    """Načte seznam URL ze souboru a vrátí je s označením stavu."""
    urls = []
    try:
        with open(filename, 'r', encoding='utf-8') as f:
            lines = f.readlines()
        ...
        for i, line in enumerate(lines):
            ...
            # zkontroluj, jestli už není označená jako zpracovaná
            if line.startswith('# DONE: '):
                continue  # přeskočí již zpracované
            elif line.startswith('# ERROR: '):
                # extrahuj URL z chybového záznamu
                url = line.replace('# ERROR: ', '')
                urls.append({'url': url, 'line_num': i, 'status': 'error'})
            else:
                urls.append({'url': line, 'line_num': i, 'status': 'pending'})
...
def main():
    ...
    # spočti kolik je čekajících
    pending_urls = [url for url in urls_to_process if url['status'] == 'pending']
    error_urls = [url for url in urls_to_process if url['status'] == 'error']
    
    print(f"[*] Celkem {len(urls_to_process)} URL: {len(pending_urls)} čekajících, {len(error_urls)} s chybami")
    
    # ulož seznam URL do textového souboru (pro záznam)
    save_urls_to_file(urls_to_process, "export_wayback/urls_from_file.txt")
    ...
    print(f"[*] Zpracovávám {len(pending_urls + error_urls)} článků...")
    
    # zpracuj čekající a chybové URL
    urls_to_retry = pending_urls + error_urls
    
    for j, url_info in enumerate(urls_to_retry, 1):
        url = url_info['url']
        line_num = url_info['line_num']
        
        try:
            print(f"    [{j}/{len(urls_to_retry)}] Zpracovávám: {url}")
            ...
            print(f"    [{j}/{len(urls_to_retry)}] ✓ OK -> {path} ({len(meta['images'])} obrázků)")
            processed_count += 1
            time.sleep(2.0)  # pauza mezi požadavky
            
        except Exception as e:
            error_msg = str(e)[:100]  # omez délku chybové zprávy
    ...
    print(f"\n[*] Dokončeno! Úspěšně: {processed_count}, Chyby: {error_count}, Celkem zpracováno: {processed_count + error_count} URL.")
    print(f"[*] Výsledky uloženy do adresáře: {OUT_DIR}")
